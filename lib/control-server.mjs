 ...
 * @param {(args: { greeting: boolean, title?: string, chatContext?: Array<{author:string,text:string}>, forcePhotoAnswer?: boolean }) => Promise<string | null>} opts.testMessage
 * @param {(...args: unknown[]) => void} opts.log
 */
export function startControlServer({ port, dataDir, uiHtmlPath, getState, setForcedGreeting, testMessage, log }) {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/state') {
        ...
        sendJson(res, 200, {
          ...
          photoQuestionAnswered: Boolean(state.photoQuestionAnswered),
      ...
      if (req.method === 'POST' && url.pathname === '/api/test-message') {
        ...
        const text = await testMessage({
          ...
          forcePhotoAnswer: Boolean(body.forcePhotoAnswer),
        });
