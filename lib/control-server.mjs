 ...
 * @param {(args: { enabled: boolean, start: string, end: string }) => void} opts.setLiveWindow
 * @param {(args: { date: string, start: string, end: string } | null) => void} opts.setLiveWindowException
 ...
 */
export function startControlServer({ port, dataDir, uiHtmlPath, getState, setForcedGreeting, setEnabledWeekdays, setCommentSettings, setLiveWindow, setLiveWindowException, testMessage, log }) {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/state') {
        ...
        sendJson(res, 200, {
          ...
          liveWindow: state.liveWindow ?? { enabled: false, start: '00:00', end: '23:59' },
          liveWindowException: state.liveWindowException ?? null,
      ...
      if (req.method === 'POST' && url.pathname === '/api/live-window') {
        const body = await readJsonBody(req);
        const enabled = Boolean(body.enabled);
        ...
        if (!hhmm.test(start) || !hhmm.test(end)) {
          sendJson(res, 400, { error: 'start e end devem estar no formato HH:MM (horário de Brasília)' });
          return;
        }
        setLiveWindow({ enabled, start, end });
        sendJson(res, 200, { liveWindow: { enabled, start, end } });
        return;
      }

      if (req.method === 'POST' && url.pathname === '/api/live-window-exception') {
        const body = await readJsonBody(req);
        const hhmm = /^([01]\d|2[0-3]):[0-5]\d$/;
        ...
        const end = typeof body.end === 'string' ? body.end : '';
        if (!date && !start && !end) {
          setLiveWindowException(null);
          sendJson(res, 200, { liveWindowException: null });
          return;
        }
        if (!isoDate.test(date) || !hhmm.test(start) || !hhmm.test(end)) {
          sendJson(res, 400, { error: 'date deve ser YYYY-MM-DD e start/end HH:MM (horário de Brasília)' });
          return;
        }
        setLiveWindowException({ date, start, end });
        sendJson(res, 200, { liveWindowException: getState().liveWindowException });
        return;
      }
